...
interp_Track<-function(x,y,id){
    ...
    breaks <- diff(non.na.idx)
    x_increment <- diff(x[non.na.idx])/breaks
    y_increment <- diff(y[non.na.idx])/breaks
    j = 0
    xx=NULL
    yy=NULL
    for(i in 1:length(x)){
      if(i %in% non.na.idx){
        ...
        xx<-c(xx,xx[i-1]+x_increment[j])
        yy<-c(yy,yy[i-1]+y_increment[j])
        
      }
    }
    new_df <- data.frame(X=xx,Y=yy,id)
    new_df
  }
...
regularize_gappy_track <- function(x,y,datetime) {
  ...
  #new_length <- diff(range(old_df$datetime)) %>% as.numeric() * 24
...
#x <- migs[[1]]
#old_df <- data.frame(x=x$X,
#                     y=x$Y,
#                     datetime=as.POSIXct(x$study.local.timestamp))
